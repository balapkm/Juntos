...
class GeneratePo extends CI_Controller 
{
	...
	public function downloadAsPdfPODetailsAction()
	{
		...
		if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') 
		{
			...
			$cmd = 'cd C:\Program Files\wkhtmltopdf\bin && wkhtmltopdf.exe '.$folder_name.'/'.$filename.'.html --footer-right "Page no :"[page]/[topage] --footer-left "'.$this->data['searchPoData'][0]['full_po_number'].'-'.date('Y',strtotime($this->data['searchPoData'][0]['po_date'])).'-'.$this->data['searchPoData'][0]['order_reference'].'" --footer-font-size 10 '.$folder_name.'/'.$filename.'.pdf  2>&1';
		}
		else
		{
			$cmd = 'xvfb-run --server-args="-screen 0, 1024x768x24" wkhtmltopdf '.$folder_name.'/'.$filename.'.html --footer-right "Page no :"[page]/[topage] --footer-left "'.$this->data['searchPoData'][0]['full_po_number'].'-'.date('Y',strtotime($this->data['searchPoData'][0]['po_date'])).'-'.$this->data['searchPoData'][0]['order_reference'].'" --footer-font-size 10 '.$folder_name.'/'.$filename.'.pdf  2>&1';
		}
